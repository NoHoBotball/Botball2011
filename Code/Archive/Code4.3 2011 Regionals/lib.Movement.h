...
// and I found a provided turn function in "InstallationPath\targets\gcc\include\kiss-create-2011.h"  around line 1056
...
void turnRight()//treat as if turning right on right side (on left side everything is reversed)
{
	if (side == Right)	turnCW();
	if (side == Left)	turnCCW();
	if (side != Left && side != Right)
	{
		printf("\nError Side is neither Right nor Left\n");
	}
...
{
	if (side == Right)	turnHalfCCW();
	if (side == Left)	turnHalfCW();
	if (side != Left && side != Right)
	{
		printf("\nError Side is neither Right nor Left\n");
	}
...
{
	if (side == Right)	turnHalfCW();
	if (side == Left)	turnHalfCCW();
	if (side != Left && side != Right)
	{
		printf("\nError Side is neither Right nor Left\n");
	}
