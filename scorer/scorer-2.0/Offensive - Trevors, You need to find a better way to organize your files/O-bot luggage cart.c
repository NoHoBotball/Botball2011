//Offensive Bot Ver 3.1.4.159//

/////-v-Define things :D-v-/////
#define motorL 1//Left motor
#define motorR 0//Right motor
#define IR analog(4) //Analog on claw
#define touch digital(12) //Touch sensor
#define threshold 150 //Treshold for IR sensor
/////-^-Define things :D-^-/////

/////-v-Intergers-v-/////
int midpoint;
/////-^-Intergers-^-/////

/////-v-Voids-v-/////
void forward(int speed);
void reverse(int speed);
void left(int tspeed);
void right(int tspeed);
void GetMidpoint();
void turnCW();
void turnCCW();
/////-^-Voids-^-/////

//track_update, track_count, track_x,track_y

int main()
{	enable_servos();
	set_servo_position(1, 0);
	printf("Press A YOU MOFO\n");
	while(a_button() == 0){}

	sleep(1);
	track_update();
	printf("%d blobs\n", track_count(2));
	while(track_size(2, 0) < 15)
	{
		left(250);
		track_update();
	}
	beep();
	track_update();
	while(track_x(2, 0) != 79 && track_size(2, 0) >= 15) //Tries to find center of green
	{
		if(track_x(2,0) > 84) //If it's on right
		{
			right(100);
			track_update();
		}
		if(track_x(2, 0) < 74) //If it's on left
		{
			left(100);
			track_update();
		}
	}
	beep(); //Centered at green
	
	sleep(1);
	
	while(touch == 0)
	{	
		int i;
		for(i = 0; 1 <= 50; i++)
		{
			forward(500);
		}
		track_update();
		if(track_x(2,0) > 84) //If it's on right
		{
			right(100);
			track_update();
		}
		if(track_x(2, 0) < 74) //If it's on left
		{
			left(100);
			track_update();
		}
	}
	
	/* For Ben, the patrolling program
	turnCCW();
	while(IR < threshold)
	{
		forward(1000);
	}
	ao();
	sleep(1);
	int i;
	for (i = 0; i <= 25; i++)
	{
		reverse(1000);
	}
	turnCW();
	while(IR < threshold)
	{
		forward(1000);
	}
	
	while(b_button() == 0)
	{
		int i;
		for (i = 0; i <= 100; i++)
		forward(500);
		for (i = 0; i <= 100; i++)
		reverse(500);
	}*/

}


void turnCW(){ 
	mav(0,-700); mav(1,700);
	sleep(0.479999);
	ao();	
}	
void turnCCW(){ 
	mav(0,700); mav(1,-700);
	sleep(0.46);
	ao();	
	
}	

void forward(int speed)
{
	mav(motorL, speed);
	mav(motorR, speed);
}

void reverse(int speed)
{
	mav(motorL, -speed);
	mav(motorR, -speed);
}

void left(int tspeed)
{
	mav(motorL, -tspeed / 2);
	mav(motorR, tspeed / 2);
}

void right(int tspeed)
{
	mav(motorL, tspeed / 2);
	mav(motorR, -tspeed / 2);
}

void GetMidpoint() //Get midpoint for the x-coordinate of the luggage carts
{
	track_update();
	midpoint = (track_x(0, 0) + track_x(2, 0)) / 2;
}
